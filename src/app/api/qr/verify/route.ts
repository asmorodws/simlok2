...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only VERIFIER and ADMIN can scan QR codes
    if (!['VERIFIER', 'ADMIN'].includes(session.user.role)) {
      return NextResponse.json({ 
        error: 'Access denied. Only verifiers and admins can scan QR codes.' 
      }, { status: 403 });
    }

    const body = await request.json();
    const { qrData, scanLocation, scanNotes } = body;
    ...
    const qrPayload: QrPayload | null = parseQrString(qrData);
    
    if (!qrPayload) {
      return NextResponse.json({ 
        success: false,
        message: 'Invalid QR code or QR code has been tampered with. Please make sure you are scanning a valid SIMLOK QR code generated by the system.' 
      }, { status: 400 });
    }

    // Check if QR is valid for current date (simplified check)
    const now = Date.now();
    ...
    const scanRecord = await prisma.qrScan.create({
      data: {
        ...
        notes: scanNotes || `Scanned at: ${scanLocation || 'Unknown location'}`,
      },
      include: {
        user: {
          select: {
