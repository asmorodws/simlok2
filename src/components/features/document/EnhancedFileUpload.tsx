...
import { ImageCompressor } from "@/utils/file/compression/imageCompression";
...
import {
  ...
  validatePDFDocument,
  validateDocument,
} from "@/utils/file/fileValidation";
...
export default function EnhancedFileUpload({
  ...
  const handleFileSelect = async (files: FileList | null) => {
    ...
    if (!validation.isValid) {
      console.warn(`[EnhancedFileUpload] ⚠️ VALIDATION FAILED - File rejected:`, validation.error);
    ...
    try {
      ...
      setTimeout(() => setUploadProgress(0), TIMEOUTS.UPLOAD_PROGRESS_RESET);
      setUploading(false);
    }
  };
